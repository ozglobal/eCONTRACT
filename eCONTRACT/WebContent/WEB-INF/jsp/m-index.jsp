...
function showReport(){
	if(isMobile.any()){
		location.href = "m-application-ozviewer?prod_no=&reportname=";
	}else{
...
function openCompletedList(){
	if(isMobile.any()){
		location.href = "m-completed-list";
	}else{
		location.href = "completed-list";	
	}
}
	
var eventHandler = function(info) {
	OZTotoFramework.indicator.stop();
}
OZTotoFramework.addEventListener("onPageLoaded", eventHandler);
</script>
