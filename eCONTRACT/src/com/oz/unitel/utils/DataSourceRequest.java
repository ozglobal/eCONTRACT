...
public class DataSourceRequest {
	...
	private static void filter(Criteria criteria, FilterDescriptor filter, Class<?> clazz) {
        if (filter != null) {
            ...
            if (!filters.isEmpty()) {
            	for(FilterDescriptor filter_dscriptor : filters){
            		if("contract_type_id".equalsIgnoreCase(filter_dscriptor.field)){
                		filter_dscriptor.setField("contract_type.contract_type_id");
                	}
                	if("group_id".equalsIgnoreCase(filter_dscriptor.field)){
                    	filter_dscriptor.setField("checkgroup.group_id");
                    	//long test = (long) filter_dscriptor.value;
                    	filter_dscriptor.setValue((long) intTolong((Integer) filter_dscriptor.value));
                    }
                	if("role_id".equalsIgnoreCase(filter_dscriptor.field)){
                		filter_dscriptor.setField("user_role.role_id");
                	}
                	if("uprice".equalsIgnoreCase(filter_dscriptor.field)){
                		filter_dscriptor.setValue(((Integer) filter_dscriptor.value).toString());
                	}
                	if("brand".equalsIgnoreCase(filter_dscriptor.field)){
                		if(filter_dscriptor.value instanceof HashMap){
                			HashMap<String, Object> map = new HashMap<String, Object>();
                			map = (HashMap<String, Object>)filter_dscriptor.value;
                			filter_dscriptor.setValue((String)map.get("text"));
                		}
                		filter_dscriptor.setField("brand.brand_name");
                	}
                	if("checkgroup".equalsIgnoreCase(filter_dscriptor.field)){
                		if(filter_dscriptor.value instanceof HashMap){
                			HashMap<String, Object> map = new HashMap<String, Object>();
                			map = (HashMap<String, Object>)filter_dscriptor.value;
                			filter_dscriptor.setValue((String)map.get("text"));
                		}
                		filter_dscriptor.setField("checkgroup.group_name");
                	}
                	
                	if("Account_user".equalsIgnoreCase(clazz.getSimpleName())){
    	            	if("company_name".equalsIgnoreCase(filter_dscriptor.field)){
    	                	filter_dscriptor.setField("company.company_name");
    	                }
    	            	if("address".equalsIgnoreCase(filter_dscriptor.field)){
    	                	filter_dscriptor.setField("company.address");
    	                }
    	            	if("tel".equalsIgnoreCase(filter_dscriptor.field)){
    	                	filter_dscriptor.setField("company.tel");
    	                }
                	}
                	
                	if("Contract".equalsIgnoreCase(clazz.getSimpleName())){
    	            	if("customer".equalsIgnoreCase(filter_dscriptor.field)){
    	                	filter_dscriptor.setField("customer.customer_name");
    	                }
                	}
                	
                	if("invoice_id".equalsIgnoreCase(filter_dscriptor.field))
                	{
                		filter_dscriptor.setField("invoice.invoice_id");
                		filter_dscriptor.setValue((long) intTolong((Integer) filter_dscriptor.value));
                	}
            	}
    ...
    private static void sort(Criteria criteria, List<SortDescriptor> sort) {
        if (sort != null && !sort.isEmpty()) {
            for (SortDescriptor entry : sort) {
                ...
                if("contract_type_id".equalsIgnoreCase(field)){
                	field = "contract_type.contract_type_id";
            	}
    ...
    public DataSourceResult toDataSourceResult(Session session, Class<?> clazz) {
        Criteria criteria = session.createCriteria(clazz);
        criteria.add(Restrictions.eq("user_role.role_id", 2));// Only ADMIN
        criteria.add(Restrictions.eq("deleted", 0));// Only ADMIN
    ...
    public static long intTolong(Integer val){
